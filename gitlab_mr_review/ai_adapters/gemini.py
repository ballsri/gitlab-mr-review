...
class GeminiAdapter(AIAdapter):
        ...
        *,
        ...
        if pricing:
            ...
            if 'flash' in model_name.lower():
                # Gemini 2.5 Flash
                self.pricing = {
                    'input_per_million': 0.3,     # $0.30 per 1M input tokens
                    'output_per_million': 2.5     # $2.50 per 1M output tokens
                }
            elif 'pro' in model_name.lower():
                # Gemini 2.5 Pro
                self.pricing = {
                    'input_per_million': 1.25,    # $1.25 per 1M input tokens
                    'output_per_million': 10.0    # $10.00 per 1M output tokens
                }
            else:
        ...
        generation_config = {
            "temperature": 0.2,  # Slightly higher for more diverse output
            "top_p": 0.95,
            "top_k": 64,  # Increased for more variety
            "max_output_tokens": 32768,  # Doubled to allow more comprehensive reviews
            "response_mime_type": "application/json",  # Force JSON output
        }
        
        # Relaxed safety settings for code review
        safety_settings = [
    ...
    def _validate_bracket_balance(original: str, replacement: str) -> Tuple[bool, str]:
        ...
        """
        orig_brackets = GeminiAdapter._count_brackets(original)
        repl_brackets = GeminiAdapter._count_brackets(replacement)
    ...
    def build_review_prompt(self, mr_data: Dict[str, Any],
        ...
        """Build prompt for Gemini review"""
        prompt = f"""You are an expert code reviewer. Review this GitLab merge request COMPREHENSIVELY and report ALL issues you observe.

🎯 CRITICAL INSTRUCTION: PROVIDE A THOROUGH, COMPLETE REVIEW
- Generate as much as you can, for the comments for a typical MR with multiple files
- Review EVERY file thoroughly from start to finish, not just the first few
- Find issues at ALL severity levels (critical, high, medium, low)
- Do NOT stop early - use the FULL token budget to review ALL code changes
- PRIORITIZE "code" suggestion_type with actual code fixes whenever possible
...
Review rules:
1. Report ALL legitimate issues you can justify across all severity levels
2. Capture every critical/high issue (mandatory)
3. Also report medium and low issues (don't skip these - they're valuable)
4. Use the MR metadata below for context when summarizing
...
🚨 CRITICAL: NEVER PUT CODE IN CONCEPTUAL SUGGESTIONS!
WRONG approach (like the image you saw):
{{
  "suggestion_type": "conceptual",  ← INCORRECT!
  "explanation": "Add validation decorators...\\n\\nexport class RegisterDto {{\\n  @IsString()\\n  username: string;\\n}}"
}}

RIGHT approach - use suggestion_type="code":
{{
  "file": "src/auth/dto/register.dto.ts",
  "start_line": 5,
  "end_line": 6,
  "suggestion_type": "code",  ← CORRECT!
  "code_fix": "  @IsString()\\n  @IsNotEmpty()\\n  username: string;",
  "explanation": "Add validation decorators from class-validator library to validate user input"
}}

REMEMBER: If showing replacement code → use "code" type with code_fix field!
Only use "conceptual" for high-level guidance WITHOUT specific code replacement!

OUTPUT FORMAT
You MUST respond with ONLY valid JSON. The response_mime_type is set to application/json, so return raw JSON without any markdown formatting.
    ...
    def review_code(self, mr_data: Dict[str, Any], 
        ...
        try:
            ...
            if not response.text:
                ...
                return {
                    "summary": "Review completed (response filtered by safety settings)",
                    "issues": []
                }, None
            ...
            if hasattr(response, 'candidates') and response.candidates:
                ...
                if finish_reason and finish_reason.name not in ['STOP', 'MAX_TOKENS']:
                    print(f"Warning: Response finished with reason: {finish_reason.name}")
            ...
            if usage_metadata:
                print(f"Token usage - Input: {usage_metadata.prompt_token_count}, Output: {usage_metadata.candidates_token_count}, Total: {usage_metadata.total_token_count}")
            
            # Parse and validate response
            parsed_response = self.parse_review_response(response.text)
